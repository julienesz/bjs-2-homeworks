...
function compareArrays(arr1, arr2) {
    if (arr1.length !== arr2.length) {
      ...
      return arr1.every((number,i) => number === arr2[i]);
}
...
function getUsersNamesInAgeRange(users, gender) {
  let result = users.filter(user => user.gender === gender).map(user => user.age).reduce((acc, age, index, filteredUsersAges) => acc + age / filteredUsersAges.length, 0);
  return result;
}
